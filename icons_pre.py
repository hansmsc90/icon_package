...
######final functions
######spatial mean
def spatial_mean(files,var,lat,lon,mask=None,better_time=True):
...
def temporal_mean(files,var,time,lat,lon,mask=None,better_time=True):
    ...
    if better_time == True :
        dsetvar = xr.open_mfdataset(files, engine='netcdf4',combine='by_coords',chunks='auto')[var]
        variable = dsetvar.assign_coords(time=fix_time(dsetvar)).chunk({'time':96,'ncells':5000000}).sel(time=slice(time[0],time[1]))
        ...
        #variable = dsetvar.chunk({'time':96,'ncells':10000000})
        variable = dsetvar.chunk({'time':96,'ncells':5000000})
    ####area
    variable = mask_coord(variable)
    ###calculate
...
######final functions
######spatial mean
def zonal_mean(files,var,lat,lon,interval,mask=None,better_time=True):
    ### mask = 0 to select ocean
    ### mask = 1 to select land
    dsetvar = xr.open_mfdataset(files, engine='netcdf4',combine='by_coords',chunks='auto')[var]
    if better_time == True :
...
def meridional_mean(files,var,lat,lon,interval,mask=None,better_time=True):
    ### mask = 0 to select ocean
    ### mask = 1 to select land
    dsetvar = xr.open_mfdataset(files, engine='netcdf4',combine='by_coords',chunks='auto')[var]
    if better_time == True :
