...
def mask_coord(dset):
        filegr = '/work/bm1235/k203123/experiments/ngc2012/run_20271101T000000-20280229T235830/bc_land_frac.nc'
...
def area_model(lat,lon,mask):
    file = '/work/bm1235/k203123/experiments/ngc2012/run_20271101T000000-20280229T235830/icon_grid_G.nc'
...
def spatial_mean(files,var,time,lat,lon,t_step='1D',mask=None,better_time=True):
    ...
    if better_time == True :
        dsetvar = xr.open_mfdataset(files, engine='netcdf4',combine='by_coords',chunks={'ncells':-1})[var]
        variable = dsetvar.assign_coords(time=fix_time(dsetvar)).sel(time=slice(time[0],time[1])).chunk({'time':30}).resample(time=t_step).mean('time')
    else:
...
def spatial_mean_save(files,var,time,lat,lon,path,outfile,t_step='1D',mask=None,better_time=True):
    ### mask = 0 to select ocean
    ### mask = 1 to select land
    if better_time == True :
        dsetvar = xr.open_mfdataset(files, engine='netcdf4',combine='by_coords',chunks={'ncells':-1})[var]
    ...
    cal_mean = calculate_spatial(variable,area,global_area,lat,lon,mask)
    ###dask
    var_final = cal_mean.compute()
    return var_final.to_netcdf(path+var+outfile+'.nc', format = 'NETCDF4', mode ='w', group=None)
